...
const Dashboard = () => {
  ...
  const stats = useMemo(() => {
    ...
    products.forEach((product) => {
      ...
      inventoryValue += purchasePrice * stock;
      sellingValue += sellingPrice * stock;
    });
      ...
      <div className="mx-auto max-w-[1600px]">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <div className="mt-3 flex items-end justify-between">
              <h2 className="text-3xl font-bold text-slate-900 dark:text-white">{stats.totalProducts}</h2>
              <div className="rounded-xl bg-blue-100 px-3 py-2 text-xl dark:bg-blue-500/10">📦</div>
            </div>
            <p className="mt-3 text-xs text-slate-400">Products in catalog</p>
          </div>

          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Total Stock</p>
            <div className="mt-3 flex items-end justify-between">
              <h2 className="text-3xl font-bold text-slate-900 dark:text-white">{stats.totalStock}</h2>
              <div className="rounded-xl bg-indigo-100 px-3 py-2 text-xl dark:bg-indigo-500/10">🏷️</div>
            </div>
            <p className="mt-3 text-xs text-slate-400">Available quantity</p>
          </div>

          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Low Stock</p>
            <div className="mt-3 flex items-end justify-between">
              <h2 className="text-3xl font-bold text-slate-900 dark:text-white">{stats.lowStock}</h2>
              <div className="rounded-xl bg-amber-100 px-3 py-2 text-xl dark:bg-amber-500/10">⚠️</div>
            </div>
            <p className="mt-3 text-xs text-amber-600 dark:text-amber-400">Needs attention</p>
          </div>

          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Out of Stock</p>
            <div className="mt-3 flex items-end justify-between">
              <h2 className="text-3xl font-bold text-slate-900 dark:text-white">{stats.outOfStock}</h2>
              <div className="rounded-xl bg-red-100 px-3 py-2 text-xl dark:bg-red-500/10">❌</div>
            </div>
            <p className="mt-3 text-xs text-red-600 dark:text-red-400">Currently unavailable</p>
          </div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <h2 className="mt-2 text-2xl font-bold text-slate-900 dark:text-white">{formatCurrency(stats.inventoryValue)}</h2>
            <p className="mt-2 text-xs text-slate-400">Purchase price × stock</p>
          </div>

          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Selling Value</p>
            <h2 className="mt-2 text-2xl font-bold text-slate-900 dark:text-white">{formatCurrency(stats.sellingValue)}</h2>
            <p className="mt-2 text-xs text-slate-400">Selling price × stock</p>
          </div>

          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Potential Margin</p>
            <h2 className={`mt-2 text-2xl font-bold ${stats.potentialMargin >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}>
              {formatCurrency(stats.potentialMargin)}
            </h2>
            <p className="mt-2 text-xs text-slate-400">Selling value − inventory value</p>
          </div>
        </div>
        ...
        <div className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="flex flex-col gap-3 border-b border-slate-200 p-5 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Latest products added to inventory</p>
            </div>
            <button
              type="button"
              ...
              <tbody>
                {recentProducts.length === 0 ? (
                  ...
                  recentProducts.map((product) => {
                    ...
                    return (
                      <tr key={product._id} className="border-b border-slate-100 transition hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800/50">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                                {product.subcategory || "Handicraft"}
                              </p>
                            </div>
                          </div>
                        </td>
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-3 grid grid-cols-3 gap-2">
                          <div>
                            ...
                            <p className="text-sm font-bold text-slate-900 dark:text-white">{Number(product.stock || 0)}</p>
                          </div>
                          <div>
                            <p className="text-[10px] text-slate-400">Purchase</p>
                            <p className="text-sm font-bold text-slate-900 dark:text-white">{formatCurrency(product.purchasePrice)}</p>
                          </div>
                          <div>
                            <p className="text-[10px] text-slate-400">Selling</p>
                            <p className="text-sm font-bold text-slate-900 dark:text-white">{formatCurrency(product.sellingPrice)}</p>
                          </div>
                        </div>
                      </div>
                    </div>
